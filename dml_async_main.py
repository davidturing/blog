...
def execute_dml_phase(blackboard_ref):
    ...
    # 强制状态扭转，唤醒主线程
    blackboard_ref.set_state(BlackboardState.STRATEGY_EXTRACTED)
...
def execute_backtest_phase(blackboard_ref):
    ...
    dml_results = blackboard_ref.get_data("dml_results")
    ...
    plt.title('SCUT DML Causal Quant Strategy Equity Curve (Event-Driven)', fontsize=14, fontweight='bold')
    plt.xlabel('Date')
    ...
    # 强制状态扭转，唤醒主线程
    blackboard_ref.set_state(BlackboardState.BACKTEST_COMPLETED)
...
class DMLUserProxyAgent(autogen.UserProxyAgent):
    ...
    async def initiate_dml_research(self, task_instruction: str, left_brain: autogen.AssistantAgent, right_brain: autogen.AssistantAgent):
        print(f"\n[{self.name}] 接收到研究员指令: {task_instruction}")
        
        print(f"[{self.name}] -> 状态流转: DATA_LOAD")
        self.blackboard.set_state(BlackboardState.DATA_LOAD)
        
        print(f"[{self.name}] -> 状态流转: DML_RUNNING")
        self.blackboard.set_state(BlackboardState.DML_RUNNING)
        
        # 为了保证 100% 确定性执行数学任务，由 Agent 内嵌函数进行实际调用
        ...
        await self.blackboard.wait_for_state(BlackboardState.STRATEGY_EXTRACTED)
        
        best_factor = self.blackboard.get_data("significant_factor")
        theta_dir = self.blackboard.get_data("causal_direction")
        print(f"[{self.name}] ✅ 左脑完成！显著因子: {best_factor} (因果方向: {theta_dir})")
        ...
        await self.blackboard.wait_for_state(BlackboardState.BACKTEST_COMPLETED)
    ...
    def generate_final_report(self):
        ...
        if metrics:
            ...
            print(f"📉 纯因果系数 Theta: {self.blackboard.get_data('causal_direction')} (负值代表反转)")
            print(f"📈 策略年化收益: {metrics.get('Annual_Return')}")
            print(f"🔥 策略夏普比率: {metrics.get('Sharpe_Ratio')}")
            print(f"🛡️ 最大回撤控制: {metrics.get('Max_Drawdown')}")
        print(">> 所有学术产出文件（CSV/PNG/MD）已重新生成覆盖，落盘至 scut_thesis_output 目录。")

...
class DMLUserProxyAgent:
    def __init__(self, name="DML_User_Proxy", blackboard=None):
        self.name = name
        self.blackboard = blackboard

    async def initiate_dml_research(self, task_instruction: str):
        ...
        print(f"[{self.name}] -> 模拟 a_initiate_chat 调用 LeftBrainAgent...")
        left_task = asyncio.create_task(
            asyncio.to_thread(execute_dml_phase, self.blackboard)
        )
        ...
        print(f"[{self.name}] ✅ 左脑状态流转确认！发现显著因子: {best_factor} (纯因果系数 Theta: {theta_val:.6f})")

        print(f"[{self.name}] -> 模拟 a_initiate_chat 唤醒 RightBrainAgent 执行回测...")
        right_task = asyncio.create_task(
            asyncio.to_thread(execute_backtest_phase, self.blackboard)
        )
        
        print(f"[{self.name}] ⏳ 挂起主控线程，严格等待右脑完成并刻入 BACKTEST_COMPLETED ...")
        await self.blackboard.wait_for_state("BACKTEST_COMPLETED")
        
        print(f"[{self.name}] ✅ 右脑状态流转确认！正在聚合课题报告...")
    ...
    def generate_final_report(self):
        ...
        if metrics:
            ...
            print(f"📉 纯因果系数 Theta: {self.blackboard.get_data('theta_value'):.6f} (负值代表反转特征)")
            print(f"📈 策略年化收益: {metrics.get('Annual_Return')}")
            print(f"🔥 策略夏普比率: {metrics.get('Sharpe_Ratio')}")
            print(f"🛡️ 最大回撤控制: {metrics.get('Max_Drawdown')}")
            print(f"📝 回测结论: DML双重机器学习成功剥离混淆特征，验证了反转因子的纯因果收益显著性！并且通过异步黑板实现了无等待错位的完美时序串联。")
...
async def main():
    ...
    await user_proxy.initiate_dml_research("开始全自动 DML 课题执行")
